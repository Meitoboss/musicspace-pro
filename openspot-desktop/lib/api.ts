...
// Lazy check: evaluate Tauri availability at call time. Production webviews can
// expose a tauri:// origin before the JS globals are fully initialized.
const isTauri = () => {
...
const fetchJioSaavn = async (endpoint: string, params: Record<string, string | number> = {}) => {
  ...
  if (isTauri()) {
    // In Tauri, use Tauri HTTP plugin fetch (bypasses tauri:// protocol issues)
  ...
  if (isWeb && canUseWebProxy()) {
    // Use local proxy API to avoid CORS on web
    const proxyUrl = new URL('/api/saavn', window.location.origin);
    proxyUrl.searchParams.append('endpoint', endpoint);
    Object.entries(params).forEach(([key, value]) => {
      proxyUrl.searchParams.append(key, String(value));
    });
    const response = await axios.get(proxyUrl.toString(), { timeout: 15000 });
    return response.data;
  }

  // Direct API call on native (no CORS issues)
